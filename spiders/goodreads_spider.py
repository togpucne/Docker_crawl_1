...
class GoodreadsSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set default MongoDB host if environment variable not found
        mongo_host = os.getenv('Mongo_HOST', 'localhost')  # Use the appropriate MongoDB host
        mongo_port = int(os.getenv('Mongo_PORT', 27017))  # Use the appropriate MongoDB port

        # Thiết lập kết nối đến MongoDB
        self.client = pymongo.MongoClient(mongo_host, mongo_port)
        self.db = self.client['goodreads_db']  # Change to your database name if different
        self.collection = self.db['books']  # Change to your collection name if different
    ...
    def parse_book_details(self, response):
        ...
        # Output data to JSON
        yield book_data  # Ensure Scrapy can still export to JSON
